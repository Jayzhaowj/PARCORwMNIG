...
#'
#' @export
shrinkTVP_mNIG <- function(formula,
                      data,
                      K,
                      niter = 10000,
                      nburn = round(niter / 2),
  ...
  #to_test_bool <- list(display_progress = display_progress,
  #                     sv = sv,
  ...
  runtime <- system.time({
    suppressWarnings({
      res <- do_shrinkTVP_mNIG(y,
                          ...
                          hyperprior_param$g0,
                          hyperprior_param$G0,
                          hyperprior_param$eta2_d1,
                          hyperprior_param$eta2_d2,
                          hyperprior_param$p_d1,
                          hyperprior_param$p_d2,
                          hyperprior_param$par_c,
                          display_progress,
                          ret_beta_nc,
                          store_burn,
                          sv,
  ...
  #residuals <- as.numeric(apply(res$residuals, 2, mean))
  residuals <- as.numeric(apply(res$residuals, 2, median))
  beta_mean <- apply(res$beta, 1:2, mean) ### dimension: (n_T+1) * K
  beta_median <- apply(res$beta, 1:2, median)
  #beta_mean <- beta_median
  #browser()
  #n_T <- nrow(beta_mean[-1, ])
  #tmp <- sapply(1:n_T, function(i) x[i, , drop = FALSE] %*% as.matrix(beta_mean[i+1, ]))
  #residuals2 <- y - tmp
  beta_qtl <- apply(res$beta, 1:2, quantile, 0.025)
  beta_qtu <- apply(res$beta, 1:2, quantile, 0.975)
  delta_mean <- apply(res$delta, 2, mean)
